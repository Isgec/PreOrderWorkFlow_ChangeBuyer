...
namespace PreOrderWorkflow_ChangeBuyer
{
    public partial class ChangeBuyer : System.Web.UI.Page
    {
        ...
        private void GetData()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                if (dt.Rows[0]["BuyerName"].ToString() != "")
                {
                    ...
                    lblBuyerName.Text = dt.Rows[0]["Buyer"].ToString();
                }
                else
                {
                ...
                if (dt.Rows[0]["Manager"].ToString() != "")
                {
                   ...
                   lblManagername.Text = dt.Rows[0]["Manager"].ToString();
                }
                else
                {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            if (ddlBuyer.SelectedValue!= "" || ddlManager.SelectedValue != "")
            {
               // string[] Buyer = txtBuyer.Text.Split('-');
                ...
                if (res > 0)
                {
                    InsertPreHistory(Convert.ToInt32(Request.QueryString["WFID"]), "Change Buyer/Manager");
                    // Response.Redirect("ReleaseTechnicalSpecification.aspx?u=" + Request.QueryString["u"]);
                    lblUpdate.Visible = true;
                    lblUpdate.Text = "Changes have been updated successfully!";
                    Response.Redirect("ProjectInProcess.aspx?u=" + Request.QueryString["u"]);
                }
            }
            else
            {
        ...
        private void GetBuyer()
        {
            ...
            ddlBuyer.DataValueField = "CardNo";//EmployeeName 
        ...
        protected void ddlBuyer_SelectedIndexChanged(object sender, EventArgs e)
        {
          ...
          lblBuyerName.Text = dt.Rows[0]["CardNo"].ToString();
        }
